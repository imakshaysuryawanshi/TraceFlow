...
- **Phase 5/6/9 placeholders (obsolete for 5)**: `backend/trace_generator/`, `backend/ai/` — module docstrings + TODO markers only, no logic.
- **Phase 5 — Java parser** (2026-02): `backend/parser/parser.py` implemented with `javalang==0.13.0`. Parses the MVP subset (vars, arithmetic, if/else, for, while, `System.out.println`, basic static methods, return) into a simplified AST of plain dicts. Auto-wraps top-level snippets in a synthetic `class Main { main() { ... } }` and remaps line numbers back to the user's source. Rejects arrays, generics, `new`, do-while, switch, try/catch, break/continue, for-each, inheritance with friendly `ParserError`. Exposed as `POST /api/parse { code }` → `{ ok, ast }` or HTTP 400 `{ detail: { message, line } }`. Tests: 22/22 pytest in `/app/backend/tests/test_parser.py`.
